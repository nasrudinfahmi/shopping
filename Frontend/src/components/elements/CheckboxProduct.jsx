...
function CheckboxProduct({ label, ariaLabel, toggleCheck, checked, id }) {
  return (
    <label htmlFor={id} className="w-max flex gap-2 items-center shrink-0 cursor-pointer *:select-none">
      <input type="checkbox" name="checkProduct" id={id} defaultChecked={checked} onClick={toggleCheck} hidden aria-label={ariaLabel} />
      <span className={`grid place-content-center w-[17px] sm:w-5 h-[17px] sm:h-5 overflow-hidden rounded border border-slate-500 ${checked ? 'bg-green-400' : 'bg-white'}`}>
